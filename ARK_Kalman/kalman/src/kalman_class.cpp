#include "kalman_class.h"
#include "matrix.h"
#include <ros/ros.h>
#include <geometry_msgs/Twist.h>
#include <nav_msgs/Odometry.h>

#define vel 0.3
#define dt .01
#define PI 3.141592654
#define theta0 PI/4

bot_prediction::bot_prediction(float x_coordinate , float y_coordinate , float theta , int id) : F(4,4) , H(4,4) , P(4,4) , R(4,4) , Q(4,4) , X(4,1) , S(4,4) , Z(4,4) , K(4,4)
{
	bot_id = id;
	if(id==1)
	{
		topic = "/robot4/odom";
	}
	else if (id == 2)
	{
		topic = "/robot5/odom";
	}
	else if (id == 3)
	{
		topic = "/robot6/odom";
	}
	else if (id == 4)
	{
		topic = "/robot7/odom";
	}
	else if (id == 5)
	{
		topic = "/robot8/odom";
	}
	else if (id == 6)
	{
		topic = "/robot9/odom";
	}
	else if (id == 7)
	{
		topic = "/robot10/odom";
	}
	else if (id == 8)
	{
		topic = "/robot11/odom";
	}
	else if (id == 9)
	{
		topic = "robot12/odom";
	}
	else if (id == 10)
	{
		topic = "/robot13/odom";
	}
	chatter_pub = n.advertise<nav_msgs::Odometry>(topic, 10);
	time = 0;
	time1 = 5;
	declare(x_coordinate , y_coordinate , theta);
}

float** bot_prediction::initialize(int row, int col)
{
	float **a ;
	a=(float**)malloc(row*sizeof(float*));

	for(int i=0 ; i < row ; i++)
    	a[i]=(float*)malloc(col*sizeof(float));

	for(int i=0 ; i < row ; i++)
    {
    	for(int j=0;j < col ; j++)
			a[i][j]=0;
    }
    
   return a;
} 

void bot_prediction::declare(float x_coordinate , float y_coordinate , float theta)
{
	
	F_mat = initialize(4,4);
	H_mat = initialize(4,4);
	P_mat = initialize(4,4);
	Q_mat = initialize(4,4);
	X_mat = initialize(4,1);
	S_mat = initialize(4,4);
	R_mat = initialize(4,4);
	Z_mat = initialize(4,4);
	K_mat = initialize(4,4);


	F_mat[0][0] = 1;
	F_mat[0][2] = dt;
	F_mat[1][1] = 1;
	F_mat[1][3] = dt;
	F_mat[2][2] = 1;
	F_mat[3][3] = 1;

	H_mat[0][0] = 1;
	H_mat[1][1] = 1;

	P_mat[0][0] = 500;
	P_mat[1][1] = 500;
	P_mat[2][2] = .3;
	P_mat[3][3] = .3;


	Q_mat[2][2] = .0019739698;
	Q_mat[3][3] = .0324408797;

	X_mat[0][0] = x_coordinate;
	X_mat[1][0] = y_coordinate;
	X_mat[2][0] = vel*cos(theta);
	X_mat[3][0] = vel*sin(theta);

	R_mat[0][0] = .3;
	R_mat[1][1] = .3;

	

	F = F.con_to_mtrx(F_mat , 4 ,4);
	H = H.con_to_mtrx(H_mat , 4, 4);
	P = P.con_to_mtrx(P_mat , 4 ,4);
	R = R.con_to_mtrx(R_mat ,4,4);
	Q = Q.con_to_mtrx(Q_mat ,4 ,4);
	X = X.con_to_mtrx(X_mat , 4,1);
	S = S.con_to_mtrx(S_mat , 4 ,4);
	Z = Z.con_to_mtrx(Z_mat , 4 ,4);
	K = K.con_to_mtrx(K_mat ,4 ,4);
}

void bot_prediction::predict()
{

	std::cout << "New X" << "\n";
  	X.show();
  	std::cout << "New P" << "\n";
  	P.show();

 	if ((int)time%20 == 0 && (time > 1)) 
 	{
 		std::cout << "Reversal 180 degree\n";
 		X.mat[2][0] = -X.mat[2][0];
 		X.mat[3][0] = -X.mat[3][0];

 		time = 0;
 		time1 = 5;
 	}

  	else if( abs(time - time1) < .1)
  	{

        std::cout << "NOISE NOISE NOISE NOISE!!!!!";

       	P=(F*P)*F.transpose() + Q;
        X=F*X;
       
        time1 += 5;
    	time+=0.85;

	}
	
	else
	{

    	P=(F*P)*F.transpose();
		X=F*X;
	}

	time+=dt;
}

void bot_prediction::publish_msg()
{
	msg1.pose.pose.position.x = X.mat[0][0];
	msg1.pose.pose.position.y = X.mat[1][0];
    msg1.twist.twist.linear.x = X.mat[2][0];
    msg1.twist.twist.linear.y = X.mat[3][0];
    msg1.twist.twist.linear.z = 0;
    msg1.twist.twist.angular.x = 0;
    msg1.twist.twist.angular.y = 0;
    msg1.twist.twist.angular.z = 0;  

    int k = 0;
    for (int i = 0; i < 6; i++)
    {
    	for (int j = 0; j < 6; ++j)
    	{
    		if(i < 4 && j < 4)
    		{
    			msg1.pose.covariance[k] = P.mat[i][j];	
    		}
    		else
    		{
    			msg1.pose.covariance[k] = 0;
    		}
    		k++;
    	}
	}	

	chatter_pub.publish(msg1);
}

void bot_prediction::collide(bot_prediction other_bot)
{
	float dis = sqrt(pow(X.mat[0][0] - other_bot.X.mat[0][0] , 2) + pow(X.mat[1][0] - other_bot.X.mat[1][0],2));
	if(dis < .7)
	{
		std::cout << "Collision\n";
 		X.mat[2][0] = -X.mat[2][0];
 		X.mat[3][0] = -X.mat[3][0];
 		other_bot.X.mat[2][0] = -other_bot.X.mat[2][0];
 		other_bot.X.mat[3][0] = -other_bot.X.mat[3][0];

 		time = 0;
 		time1 = 5;
	}
}

int main(int argc, char** argv)
{
	
	
	ros::init(argc , argv , "kalman1");
	bot_prediction bot1(0 , 0 , theta0,1);
	bot_prediction bot2(2,2,theta0*5,2);
	ros::Rate loop_rate(50);

	while(ros::ok())
	{

		bot1.publish_msg();
		bot2.publish_msg();
		bot1.predict();
		bot2.predict();
		bot1.collide(bot2);
		ros::spinOnce();
	    loop_rate.sleep();
		
		printf("%s %f\n","time" ,bot1.time);
	}
	return 0;
}